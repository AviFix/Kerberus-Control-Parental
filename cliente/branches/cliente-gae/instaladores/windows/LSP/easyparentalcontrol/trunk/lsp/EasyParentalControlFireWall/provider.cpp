...
{
    HeapFree( GlobalVars::gLspHeap, 0, buf );
}
...
{
    GlobalVars::gLspHeap = HeapCreate( 0, 128000, 0 );
    if ( NULL == GlobalVars::gLspHeap )
    {
    ...
    )
{
    if ( NULL != GlobalVars::gLspHeap )
    {
        HeapDestroy( GlobalVars::gLspHeap );
        GlobalVars::gLspHeap = NULL;
    }
}
        ...
        )
{
    ...
    *lspProviders = NULL;
    ...
    {
        if ( 0 == memcmp( &ProtocolInfo[ i ].ProviderId, &GlobalVars::gProviderGuid, sizeof( GlobalVars::gProviderGuid ) ) )
        {
